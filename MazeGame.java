...
public class MazeGame extends GameBase
{
    ...
    //Creating a rectangle for finish line and win condition ----------------
    Rect finish;
    boolean gameWon = false;
    
    //Timer -------------------
    long startTime;
    int timeLimit = 90; // seconds
    boolean gameOver = false; //checks if the timer has run out causing a game over
    ...
    public void initialize()
    {
        ...
        //Modifying size of finish line rectangle ------------------
        int size = maze.getCellSize() / 2;
    ...
    {
    	//Checks if sprite is inside the finish line -------------------
    	if (!gameWon && finish.overlaps(s))
    	{
    	    gameWon = true;
    	}
    ...
    public void paint(Graphics g)
    {
        ...
        //Draws the finish line green ---------------
        int fx = (int)(finish.x - Camera.x);
        int fy = (int)(finish.y - Camera.y);

        g.setColor(Color.GREEN);
        g.fillRect(fx, fy, finish.w, finish.h);
        
        //Visual Timer and win/loss message --------------
        long elapsed = (System.currentTimeMillis() - startTime) / 1000;
        long remaining = Math.max(0, timeLimit - elapsed);

        g.setColor(Color.WHITE);
        g.drawString("Time: " + remaining, 20, 40);

        if (gameOver)
        {
            g.setColor(Color.RED);
            g.drawString("YOU LOSE!", screenW / 2 - 40, screenH / 2);
        }

        if (gameWon)
        {
            g.setColor(Color.GREEN);
            g.drawString("YOU WIN!", screenW / 2 - 40, screenH / 2);
        }
    }
}